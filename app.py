...
def save_data(df):
    try:
        ...
        st.error(f"Kritiskt fel vid sparande: {e}")
        return False
...
if menu == "🔍 Sök & Inventarie":
    ...
    if total > 0:
        ...
        for idx, row in filtered_df.iterrows():
            ...
            r_status.markdown(f"<span style='background-color:{st_color}; padding:4px 8px; border-radius:10px; font-size:12px;'>{row['Status']}</span>", unsafe_allow_html=True)
